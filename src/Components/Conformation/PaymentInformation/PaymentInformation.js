...
import VegIcon from "../../../Asserts/ApplicationStatus/Veg icon.png";
import NonVegIcon from "../../../Asserts/ApplicationStatus/Non veg icon.png";
import {
  Table,
  TableHead,
  TableRow,
...
import * as apiService from "../../../Bakend/ConfirmationApis";
...
const PaymentInformation = ({ onBack, onSubmit, saleData = null, handleBack, applicationNo }) => {
  ...
  const [admissionNo, setAdmissionNo] = useState(
    applicationNo || location.state?.applicationNo || saleData?.applicationNo || ""
  );

  // Use campusId from saleData or fallback to a default
  const campusId = saleData?.campusId || location.state?.campusId || 1;
